...
ACMD(do_look)
{
  ...
  if (GET_POS(ch) < POS_SLEEPING)
    ...
    send_to_char(ch, "Здесь темнота, однако.\r\n");
...
ACMD(do_inventory)
{
  send_to_char(ch, "Вы несёте в инвентаре:\r\n");
...
ACMD(do_equipment)
{
  ...
  send_to_char(ch, "Вы используете (надето и держите):\r\n");
  for (i = 0; i < NUM_WEARS; i++) {
    if (GET_EQ(ch, i)) {
      ...
      if (CAN_SEE_OBJ(ch, GET_EQ(ch, i))) {
        ...
        send_to_char(ch, "%s", wear_where[i]);
        send_to_char(ch, "Что-то.\r\n");
      }
    }
  }
  if (!found)
    send_to_char(ch, " Ничего.\r\n");
}
...
ACMD(do_time)
{
  ...
  send_to_char(ch,"Время %d ч. День недели %s.\r\n", time_info.hours, weekdays_r[weekday]);
#else
  ...
  send_to_char(ch, "День %d, %s, год %d.\r\n",
	  day, month_name_r[time_info.month], time_info.year);
}
...
ACMD(do_weather)
{
  ...
  if (OUTSIDE(ch))
    {
    send_to_char(ch, "Небо %s и %s.\r\n", sky_look_r[weather_info.sky],
	    weather_info.change >= 0 ? "тёплый южный ветер" :
	     "нет ничего лучше плохой погоды");
    if (GET_LEVEL(ch) >= LVL_GOD)
      send_to_char(ch, "Давление: %d (change: %d), Небо: %d (%s)\r\n",
    ...
    send_to_char(ch, "Отсюда вы не видите состояние погоды.\r\n");
}
