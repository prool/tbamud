...
struct attack_hit_type attack_hit_text_r[] =
{
  {"ударить", "ударить"},
  {"ударить", "ударить"},
  {"ударить", "ударить"},
...
static void make_corpse(struct char_data *ch)
{
  ...
  snprintf(buf2, sizeof(buf2), "Труп %s лежит здесь.", GET_NAME(ch));
  corpse->description = strdup(buf2);

  snprintf(buf2, sizeof(buf2), "труп %s", GET_NAME(ch));
...
void death_cry(struct char_data *ch)
{
  ...
  act("Кровушка застыла у вас в жилах от предсмертного крика $n!", FALSE, ch, 0, 0, TO_ROOM);

  for (door = 0; door < DIR_COUNT; door++)
    if (CAN_GO(ch, door))
      send_to_room(world[IN_ROOM(ch)].dir_option[door]->to_room, "Ваша кровь застыла в жилах, когда вы услышали чей-то предсмертный крик!\r\n");
}
			     ...
			     struct char_data *victim)
{
  ...
  if (share > 1)
    send_to_char(ch, "Вы получили свою долю опыта -- %d единиц.\r\n", share);
  else
    send_to_char(ch, "Вы получили свою долю опыта - всего лишь одно жалкое очко!\r\n");
...
static void solo_gain(struct char_data *ch, struct char_data *victim)
{
  ...
  if (exp > 1)
    send_to_char(ch, "Вы получили %d очков опыта.\r\n", exp);
  else
    send_to_char(ch, "Вы получили одно несчастное очко опытаr!\r\n");
		      ...
		      int w_type)
{
  ...
  static struct dam_weapon_type {
    ...
    {
      "$n попробовал #w $N, но промахнулся.",	/* 0: 0     */
      "Вы попробовали #w $N, но промахнулись.",
      "$n попробовал #w вас, но не попал."
    },

    {
      "$n оцарапал $N пытаясь $e #W $M.",	/* 1: 1..2  */
      "Вы оцарапали $N пытаясь #w $M.",
      "$n поцарапал вас, пытаясь $e #W."
    },
    ...
    {
      "$n #W $N очень сильно.",		/* 5: 11..14  */
      "Вы #w $N очень сильно.",
      "$n #W вас очень сильно."
    },

    {
      "$n #W $N исключительно сильно.",	/* 6: 15..19  */
      "Вы #w $N исключительно сильно.",
      "$n #W вас исключительно сильно."
    },

    {
      "$n порубил $N в капусту $s #w.",	/* 7: 19..23 */
      "Вы порубили $N в капусту #w.",
      "$n порубил вас в капусту $s #w."
    },

    {
      "$n сокрушил $N with $s смертельно #w!!",	/* 8: > 23   */
      "Вы сокрушили $N смертельно #w!!",
      "$n сокрушил вас $s смертельно #w!!"
    }
  };
  ...
  buf = replace_string(dam_weapons[msgnum].to_room,
	  attack_hit_text_r[w_type].singular, attack_hit_text_r[w_type].plural);
...
int damage(struct char_data *ch, struct char_data *victim, int dam, int attacktype)
{
  ...
  case POS_MORTALLYW:
    act("$n смертельно ранен и умрет, если ему никто не поможет.", TRUE, victim, 0, 0, TO_ROOM);
    send_to_char(victim, "Вы смертельно ранены и умрете, если вам никто не поможет.\r\n");
    break;
  case POS_INCAP:
    act("$n недееспособен и медленно умирает.", TRUE, victim, 0, 0, TO_ROOM);
    send_to_char(victim, "Вы медленно умираете.\r\n");
    break;
  case POS_STUNNED:
    act("$n оглушен, но может скоро очнуться.", TRUE, victim, 0, 0, TO_ROOM);
    send_to_char(victim, "Вас оглушили, но вы еще можете прийти в себя.\r\n");
    break;
  case POS_DEAD:
    act("$n МЕРТВ!", FALSE, victim, 0, 0, TO_ROOM);
    send_to_char(victim, "Вы мертвы. Такие дела.\r\n");
    break;

  default:			/* >= POSITION SLEEPING */
    if (dam > (GET_MAX_HIT(victim) / 4))
      send_to_char(victim, "Это было реально больно!\r\n");
